class HomesController < ApplicationController
	...
	def top
    	...
    	@recitments = Recitment.where('runtime > ?', Time.zone.today).where(['comment LIKE ? or practice LIKE ? or title LIKE ?', "%#{search}%", "%#{search}%", "%#{search}%"]).order(created_at: :desc).page(params[:page]).per(PER)
